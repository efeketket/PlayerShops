...
tag @s add customer
data modify entity @e[type=item_display,sort=nearest,limit=1,tag=shopdata] item.tag.SellingHolder set from entity @e[type=item_display,sort=nearest,limit=1,tag=shopdata] item.tag.Price
data remove entity @e[type=item_display,sort=nearest,limit=1,tag=shopdata] item.tag.SellingHolder.Count
...
execute store result score @e[type=interaction,tag=playershop,tag=main,limit=1,distance=..1,sort=nearest] boolean_shop run data modify entity @e[type=item_display,sort=nearest,limit=1,tag=shopdata] item.tag.SellingHolder set from storage payment_holder SelectedItem
...
execute as @e[type=interaction,tag=playershop,tag=main,limit=1,distance=..1,tag=!empty,sort=nearest] at @s if score @s currentstock < @s product_count run tag @s add empty
execute as @e[type=interaction,tag=playershop,tag=main,limit=1,distance=..1,tag=empty,sort=nearest] at @s run data modify entity @e[type=text_display,limit=1,tag=displaytext,sort=nearest] text set value '{"bold":true,"color":"red","text":"Out of Stock!"}'
